...
class BaseWorker(QThread):
    ...
    def __init__(self, driver, params, data_queue):
        super().__init__()
        self.driver = driver
        self.params = params
        self.data_queue = data_queue  # 保留用于兼容原架构，实际也可直接用信号
        self.running = False
        self.start_time = 0
        self.last_p = 0
        self.last_c = 0.0
    ...
    def run(self):
        ...
        self.data_signal.emit(("CLEAR",))  # 清空曲线
...
class PurePositionWorker(BaseWorker):
    def _setup_mode(self):
        ...
        self.driver.write_reg(0x0080, (cur & 0xFF00) | 0x03)  # 外接测速闭环模式
...
class ClosedSpeedWorker(BaseWorker):
    def _setup_mode(self):
        self.driver.write_reg(0x0068, 2)  # 编码器反馈
        self.driver.write_reg(0x0069, self.params["encoder_lines"])
        self.driver.write_reg(0x0081, 0)  # 禁用堵转
        self.driver.write_reg(0x00C1, 0)  # 速度闭环模式
        cur = self.driver.read_reg(0x0080, signed=False)
        self.driver.write_reg(0x0080, (cur & 0xFF00) | 0x03)  # 外接测速闭环模式
        self.driver.write_reg(0x0046, 20000)  # 放开速度上限（可选）

    def _run_loop(self):
        target_rpm = int(self.params["target_rpm"])
        while self.running:
            self.driver.write_reg(0x0040, target_rpm, signed=True)  # 写入目标转速
            p, c, s = self.read_telemetry()
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # ========== 新增：位置清零按钮 ==========
        self.btn_clear_pos = QPushButton("位置清零")
        self.btn_clear_pos.setStyleSheet("background-color: transparent;")  # 不要颜色
        self.btn_clear_pos.clicked.connect(self.clear_position)
        realtime_layout.addWidget(self.btn_clear_pos, 2, 0, 1, 4)  # 占据整行
    ...
    def update_live_params(self):
        ...
        if isinstance(self.current_worker, FlappingWorker):
            ...
            self.current_worker.params["target_duty"] = self.speed_target_duty.value()
    ...
    def start_motion(self, mode):
        ...
        if mode == "pure_pos":
            ...
            worker = PurePositionWorker(self.driver, params, None)
        elif mode == "flapping":
            params = {
            ...
            worker = SpeedWorker(self.driver, params, None)
        elif mode == "torque":
            params = {
            ...
            worker = TorqueWorker(self.driver, params, None)
        else:
            return
    ...
    def stop_motion(self):
        ...
        if self.current_worker:
            ...
            self.current_worker.wait(500)  # 等待最多0.5秒
        # 硬件急停
    ...
    def clear_position(self):
        ...
        if not self.driver.is_connected():
            ...
            return
        try:
            self.driver.write_reg(0x700A, 1)  # 写入1清零位置计数
            self.on_log("✅ 编码器位置已清零")
            # 可选：立即读取一次位置确认（数据采集线程会自动更新显示）
